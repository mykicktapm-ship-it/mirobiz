...
import type { BusinessEvent, Invoice, Lead, Notification, Payment, Role, User } from '../domain/types';
...
export interface Repository<T> { list(workspaceId: string): T[]; get(id: string): T|undefined; }
...
class ArrayRepository<T extends {id:string; workspaceId?: string}> implements Repository<T> { constructor(private rows:T[]){} list(workspaceId:string){return this.rows.filter(r=>!r.workspaceId||r.workspaceId===workspaceId)} get(id:string){return this.rows.find(r=>r.id===id)} }
...
export class LocalNotificationProvider implements NotificationProvider { private rows=[...seed.notifications]; send(input: Omit<Notification,'id'|'createdAt'|'read'>){ const n={...input,id:`n${this.rows.length+1}`,read:false,createdAt:new Date().toISOString()}; this.rows.unshift(n); console.info('[local notification]', n.title); return n;} list(workspaceId:string){return this.rows.filter(n=>n.workspaceId===workspaceId)} }
export class MockPaymentProvider implements PaymentProvider { pay(invoice: Invoice): Payment { return {id:`pay_${invoice.id}`, invoiceId: invoice.id, amount: invoice.total, status:'mock_authorized', providerRef:'local-mock'}; } }
